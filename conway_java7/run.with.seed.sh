...
TIME_RECORD="timeRecord_java22.txt"     # File to record timing data.
...
while IFS= read -r seed; do
  ...
  #output=$( { time ./runmvn.sh -d -h 50 -w 50 -t 60 -q -f"${fname}" -s "${seed}" -j; } 2>&1 )
  output=$( { time ./runmvn.sh -h 50 -w 50 -t 120 -q -f "${fname}" -s "${seed}" -c; } 2>&1 )
